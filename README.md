...
### 2. 上传中文字体文件
为了正常显示中文，需要准备一个中文字体文件（如黑体、微软雅黑，后缀为 `.ttf`）。
1. 将准备好的字体文件重命名为 **`font.ttf`** （注意必须全部为小写）。
2. 在你的 GitHub 仓库首页，点击右上角 **`Add file`** -> **`Upload files`**。
3. 将 `font.ttf` 上传，并点击绿色的 **`Commit changes`** 按钮保存。
