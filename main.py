...
# 导入hl.py相关依赖（保持hl.py代码不变，直接内嵌核心逻辑）
import datetime as hl_datetime
...
def task_huangli():
    ...
    chong_str = f"沖{chong}" if chong else "無沖"

    yi = "、".join(lunar.getDayYi()) or "諸事不宜"
    ji = "、".join(lunar.getDayJi()) or "百無禁忌"
    ...
    # 宜：智能缩放 + 不重叠 + 留空白
    draw.text((30, 175), "宜：", font=get_font(19), fill=0)
    render_auto(draw, 65, 175, yi, 315, 2, 19, 28)

    # 忌：智能缩放 + 不重叠 + 留空白
    draw.text((30, 230), "忌：", font=get_font(19), fill=0)
    render_auto(draw, 65, 230, ji, 315, 2, 19, 28)
    
    # 推送黄历到第二页
    push_image(img, 2)
    print("✅ 终极完美版生成：不重叠 + 智能缩放 + 清晰字体")
